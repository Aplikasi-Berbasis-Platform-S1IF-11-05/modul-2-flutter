...
## Dasar Teori
Flutter adalah framework open-source yang dikembangkan oleh Google untuk membangun aplikasi mobile, web, dan desktop menggunakan satu basis kode (single codebase). Flutter menggunakan bahasa pemrograman Dart dan menyediakan berbagai widget yang memudahkan pengembang dalam membuat antarmuka aplikasi yang interaktif, responsif, dan modern. Dengan konsep widget sebagai komponen utama, Flutter memungkinkan setiap bagian tampilan seperti teks, tombol, gambar, hingga layout dapat dikustomisasi sesuai kebutuhan aplikasi.

Pada pembuatan aplikasi SiAkad LMS ini digunakan konsep antarmuka berbasis widget untuk menampilkan informasi tugas, kuis, dan deadline mahasiswa secara terstruktur. Widget seperti `Scaffold`, `AppBar`, `Container`, `GridView`, `ListView`, dan `Card` dimanfaatkan untuk membangun tampilan dashboard yang rapi dan responsif. Selain itu, Flutter juga mendukung penggunaan tema dan warna sehingga tampilan aplikasi dapat dibuat lebih konsisten dan menarik secara visual.

Aplikasi ini menerapkan konsep layout responsif menggunakan `GridView` dan `ListView` agar data tugas dapat ditampilkan dalam bentuk grid maupun daftar. Penggunaan `GridView` memungkinkan beberapa data ditampilkan berdampingan, sedangkan `ListView` digunakan untuk menampilkan daftar tugas secara vertikal dengan kemampuan scroll otomatis. Dengan penerapan konsep tersebut, pengguna dapat melihat informasi akademik secara lebih mudah, cepat, dan efisien.

## Task 2 Flutter Mobile
...
final List<Assignment> allAssignments = [
  ...
  Assignment(
    ...
    status: 'sedang dikerjakan',
    description: 'Implementasi merge sort, quick sort, dan heap sort dalam Python',
    isOnline: true,
  ),
  Assignment(
    id: '7',
    ...
    status: 'belum dikerjakan',
    description: 'Penetration testing pada web aplikasi dummy menggunakan OWASP',
    isOnline: true,
  ),
  Assignment(
    id: '8',
    ...
    status: 'belum dikerjakan',
    description: 'Presentasi kelompok tentang perbandingan kernel Linux dan Windows',
    isOnline: false,
  ),
  Assignment(
    id: '9',
  ...
  Assignment(
    ...
    status: 'belum dikerjakan',
    description: 'Kuis materi teori graf, kombinatorika, dan logika proposisional',
    isOnline: true,
  ),
];
...
``` yaml
name: lms_app
description: SiAkad LMS - Tampilan tugas dan deadline kampus
publish_to: 'none'
version: 1.0.0+1
...
<img src="Output1.2.png" alt="Keterangan Foto" width="100%">
<img src="Output1.3.png" alt="Keterangan Foto" width="100%">
<img src="Output2.png" alt="Keterangan Foto" width="100%">

### Penjelasan Code

kode program dalam `main.dart` berfungsi sebagai pusat pengaturan tampilan dan logika program. Pada bagian awal terdapat fungsi `main()` yang menjalankan aplikasi menggunakan `runApp()` dengan widget utama bernama `LMSApp`. Widget tersebut menggunakan `MaterialApp` untuk mengatur tema, judul aplikasi, jenis font, serta halaman utama aplikasi yaitu HomePage. Di dalam kode juga terdapat class Assignment yang digunakan sebagai model data untuk menyimpan informasi tugas seperti judul, mata kuliah, deadline, jenis tugas, status pengerjaan, dan deskripsi tugas. Selain itu, disediakan data contoh berupa daftar tugas yang ditampilkan menggunakan widget `GridView dan ListView`. Tampilan antarmuka dibuat menggunakan berbagai widget Flutter seperti `Scaffold, AppBar, Container, Row, Column, dan Card` sehingga menghasilkan dashboard LMS yang modern dan responsif. Penggunaan warna, ikon, serta pengelompokan tugas berdasarkan deadline membantu pengguna melihat informasi akademik dengan lebih terstruktur dan menarik.

File `pubspec.yaml` merupakan file konfigurasi utama pada proyek Flutter yang digunakan untuk mengatur informasi aplikasi dan dependency yang dibutuhkan. Pada bagian name dan description ditentukan identitas aplikasi yaitu `lms_app` dengan deskripsi aplikasi LMS untuk menampilkan tugas dan deadline kampus. Bagian environment digunakan untuk menentukan versi minimum Dart SDK dan Flutter SDK agar aplikasi dapat berjalan dengan kompatibel. Pada bagian dependencies terdapat library utama Flutter serta `cupertino_icons` yang digunakan untuk menyediakan ikon iOS. Sementara itu, bagian `dev_dependencies` berisi library pendukung pengembangan seperti `flutter_test` untuk pengujian aplikasi dan `flutter_lints` untuk membantu menjaga kualitas penulisan kode. Bagian flutter dengan `uses-material-design: true` digunakan agar aplikasi dapat menggunakan komponen desain bawaan Material Design dari Flutter.