...
Flutter adalah framework open-source dari Google untuk membangun aplikasi lintas platform seperti mobile, web, dan desktop dengan satu basis kode. Flutter menggunakan bahasa Dart dan konsep widget sebagai elemen utama penyusun tampilan, sehingga antarmuka dapat dibuat lebih cepat, konsisten, dan responsif.

### 2. Widget

Dalam Flutter, hampir semua bagian antarmuka merupakan widget, mulai dari teks, tombol, gambar, layout, hingga layar secara keseluruhan. Widget bersifat composable, artinya sebuah tampilan kompleks dapat dibangun dari gabungan banyak widget kecil. Pada tugas ini, widget digunakan untuk menyusun header, kartu tugas, grid, dan daftar tugas.

### 3. GridView

GridView adalah widget untuk menampilkan data dalam bentuk baris dan kolom. Widget ini cocok digunakan saat informasi ingin ditampilkan secara ringkas dan visual, misalnya pada kartu tugas yang paling dekat deadline. Pada implementasi tugas ini, GridView digunakan untuk menampilkan dua kartu tugas di sisi kanan dan kiri agar tampilan lebih rapi dan mudah dipindai oleh pengguna.

### 4. ListView.builder dan ListView.separated

ListView digunakan untuk menampilkan data secara vertikal dalam bentuk daftar yang dapat di-scroll. Varian ListView.builder cocok untuk data dinamis karena item hanya dibuat saat dibutuhkan, sehingga lebih efisien. ListView.separated menambahkan pemisah antaritem agar daftar terlihat lebih jelas. Pada tugas ini, ListView digunakan untuk menampilkan 8 tugas lainnya yang tidak masuk ke grid utama.

### 5. Layout dan Scroll

Layout pada Flutter dibangun menggunakan kombinasi widget seperti Column, Container, Padding, dan SingleChildScrollView. Penggunaan SingleChildScrollView penting ketika layar berisi banyak komponen agar seluruh konten tetap bisa diakses dengan scrolling. Dengan pengaturan ini, dashboard LMS menjadi lebih fleksibel dan nyaman ditampilkan di layar smartphone.

### 6. Penerapan pada Task Dashboard

Pada aplikasi ini, GridView digunakan untuk menampilkan dua tugas dengan deadline paling dekat, sedangkan ListView menampilkan delapan tugas lainnya beserta nama tugas, mata kuliah, deadline, dan data tambahan. Kombinasi kedua widget tersebut sesuai dengan kebutuhan tampilan LMS yang menekankan prioritas tugas dan daftar tugas secara terstruktur.
...
### Penjelasan Kode
Aplikasi ini sekarang menampilkan dashboard LMS dengan 2 kartu grid untuk tugas yang paling dekat deadline, lalu 8 item daftar tugas lain menggunakan GridView dan ListView.builder/separated sesuai ketentuan. Tampilan juga sudah dibuat lebih rapi dan informatif dengan judul tugas, mata kuliah, deadline, dan data tambahan pada setiap item.

### Output

<img alt="Screenshot Tampilan LMS App" src="dashboard.PNG" />
